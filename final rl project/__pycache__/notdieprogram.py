...
import time
import os
print("this is a specialized no die program")
...
def test_model(model, env_name, episodes=5):
    ...
    action_translater = {0:1,1:1,2:4,3:5,4:4,5:5}
    for episode in range(1, episodes+1):
        ...
        while not done:
            ...
            action = action_translater[action]
...
#found this in tensorforce was recommend by gymnasium
...
def train():
    # Configure GPU for Mac Metal
    try:
        physical_devices = tf.config.list_physical_devices('GPU')
        if physical_devices:
            # For Metal GPU on Mac, don't set memory growth as it's not supported
    ...
    online_net.save('models/dqn_space_invaders.keras')
